...
class GMM_EM:
    ...
    """
        ...
        numerator = pi_k * MVN(x_n | mu_k, sigma_k)
        denominator = sum[1:K](pi_i * MVN(x_n | mu_i, sigma_i))
    ...
    """
        ...
        mu_k = sum[1:N](Z[n,k]*x_n) / N_k
        sigma_k = sum[1:N](Z[n,k] * (x_n - mu_k) * (x_n - mu_k).T) / N_k
        pi_k = N_k / N   
    """
    def maximization(self):
        for i in range(self.K):
            ...
            total = response.sum()
            self.mu[i] = (self.data * response).sum(axis=0) / total
            self.sigma[i] = np.cov(self.data.T, aweights=(response / total).flatten(), bias=True)
    ...
    def main(self):
        for i in range(self.max_ite):
            ...
            # gp.GMM_data_display(self.data, self.label, self.mu, self.sigma, 'Iteration ' + str(i))
